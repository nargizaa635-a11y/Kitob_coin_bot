...
# ================== ADMIN PANEL: BO'LIM (KATEGORIYA) MENYUSI ==================
# Har bir bo'lim bosilganda o'sha bo'limga tegishli BARCHA buyruqlar ro'yxati chiqadi.
# "buttons" — bo'limga tegishli, allaqachon mavjud bo'lgan callback (masalan
# admin_randomizer) orqali bitta bosishda ishlaydigan tezkor amallar.

ADMIN_CATEGORIES = {
    "kanal": {
        "emoji": "📢",
        "title": "Majburiy kanal",
        "commands": [
            ("Kanal qo'sh", "/kanal_qosh @kanal_nomi"),
            ("Kanal ochir", "/kanal_ochir @kanal_nomi"),
            ("Kanallar ro'yxati", "/kanallar"),
        ],
    },
    "koin": {
        "emoji": "💰",
        "title": "Koin",
        "commands": [
            ("Koin qo'sh (o'zingizga)", "/addcoins 500"),
            ("Koin qo'sh (boshqa foydalanuvchiga)", "/addcoins 123456789 500"),
            ("Koin ayirish (manfiy son)", "/addcoins 123456789 -500"),
        ],
    },
    "premium": {
        "emoji": "⭐",
        "title": "Premium",
        "commands": [
            ("Premium berish (standart muddat)", "/addpremium 123456789"),
            ("Premium berish (kun ko'rsatib)", "/addpremium 123456789 14"),
            ("1 oylik buyurtmani yakunlash", "/complete_premium_5"),
        ],
        "buttons": [[InlineKeyboardButton(text="📋 Kutilayotgan buyurtmalar", callback_data="admin_premium_orders")]],
    },
    "kitob": {
        "emoji": "📚",
        "title": "Kitob",
        "commands": [
            ("Yangi kitob qo'shish", "/addbook"),
        ],
        "note": "🚧 \"Kitob o'chir\" buyrug'ini qo'shish uchun <b>database.py</b> fayli kerak — "
                "yuborsangiz, shu bo'limga qo'shib beraman.",
    },
    "vazifa": {
        "emoji": "✅",
        "title": "Vazifa",
        ...
        "note": "🚧 \"Vazifa o'chir\" buyrug'ini qo'shish uchun <b>database.py</b> fayli kerak.",
    },
    "quiz": {
        "emoji": "🧠",
        "title": "Test (Quiz)",
        "commands": [
            ("Yangi test qo'shish (.txt fayl)", "/addquiz"),
        ],
        "note": "🚧 \"Test o'chir\" buyrug'ini qo'shish uchun <b>database.py</b> fayli kerak.",
    },
    "dokon": {
        "emoji": "🛍",
        "title": "Do'kon",
        "commands": [
            ("Mahsulot qo'shish", "/addshop"),
            ("Mahsulotlar ro'yxati / narxlari", "/tahrirla_dokon"),
            ("Narxni o'zgartirish", "/narx_ozgartir 5 4.5"),
            ("Mahsulotni o'chirish", "/mahsulot_ochir 5"),
            ("Zaxirani to'ldirish", "/dokon_stok 3 20"),
            ("Buyurtmalar ro'yxati", "/buyurtmalar"),
        ],
    },
    "sandiq": {
        "emoji": "📦",
        "title": "Sandiq",
        "commands": [
            ("Sandiqlar ro'yxati", "/sandiqlar"),
            ("Yangi sandiq (yo'riqnoma)", "/sandiq_qosh"),
            ("Sandiq yaratish", "/sandiq_yarat id|nom|emoji|narx|limit|coins:min-max:foiz"),
            ("Sandiqni o'chirish", "/sandiq_ochir id"),
        ],
    },
    "tanlov": {
        "emoji": "🏆",
        "title": "Tanlov (Haftalik)",
        "commands": [
            ("Yangi tanlov yaratish", "/tanlov"),
            ("Tanlov holati", "/tanlov_holati"),
        ],
        "buttons": [[InlineKeyboardButton(text="🎲 G'olibni aniqlash", callback_data="admin_randomizer")]],
    },
    "hisobot": {
        "emoji": "📊",
        "title": "Hisobot",
        "commands": [
            ("Umumiy statistika", "/statistika"),
            ("Moliyaviy hisobot", "/moliya"),
        ],
        "buttons": [[InlineKeyboardButton(text="📊 Tezkor statistika", callback_data="admin_stats")]],
    },
    "boshqa": {
        "emoji": "🔧",
        "title": "Boshqa",
        "commands": [
            ("Sinov uchun referral qo'shish", "/addrefs 5"),
            ("Profil rasmini o'rnatish", "/avatar"),
            ("Jarayonni bekor qilish", "/bekor"),
            ("Telegram ID'ni ko'rish", "/myid"),
        ],
    },
}
...
def admin_main_keyboard() -> InlineKeyboardMarkup:
    items = list(ADMIN_CATEGORIES.items())
    rows = []
    for i in range(0, len(items), 2):
        row = []
        for key, cat in items[i:i + 2]:
            row.append(InlineKeyboardButton(text=f"{cat['emoji']} {cat['title']}", callback_data=f"admcat_{key}"))
        rows.append(row)
    return InlineKeyboardMarkup(inline_keyboard=rows)


def admin_category_text_and_keyboard(key: str):
    cat = ADMIN_CATEGORIES[key]
    ...
    text = "\n\n".join(lines)
    buttons = [row[:] for row in cat.get("buttons", [])]
    buttons.append([InlineKeyboardButton(text="◀️ Orqaga", callback_data="admcat_back")])
...
async def callback_admin_category(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Ruxsat yo'q", show_alert=True)
        return

    key = callback.data.split("admcat_", 1)[1]
    if key == "back":
        await callback.message.edit_text(
    ...
    if key not in ADMIN_CATEGORIES:
        await callback.answer()
        return

    text, keyboard = admin_category_text_and_keyboard(key)
...
# ================== STARS / PREMIUM TO'LOVLARI (faqat ilova ichida) ==================
# /buy_premium va /buy_coins chat-buyruqlari olib tashlandi — Stars orqali koin/premium
# sotib olish endi FAQAT Mini App (ilova) ichidan amalga oshiriladi
# (qarang: api_buy_stars_coins, api_buy_stars_premium). Quyidagi handlerlar ilova
# ichidan qilingan to'lovlarni qabul qilish uchun ishlatilgani sababli o'zgarishsiz qoldi.
...
async def cmd_sandiq_qosh(message: Message):
    ...
    await message.answer(
        ...
        "Mukofot formati: <code>coins:min-max:foiz</code> yoki <code>premium:kun:foiz</code>\n\n"
        "Misol:\n"
        "<code>/sandiq_yarat kumush|Kumush sandiq|🥈|70|4|coins:40-100:80,premium:1:20</code>"
    )
    ...
    coins_list = [
        {
            ...
            "user_id": r["user_id"],
            "avatar_url": f"/api/avatar/{r['user_id']}" if _safe_get(r, "avatar_path") else None,
        }
        for r in coins_rows
    ]
    referral_list = [
...
# ---------- STARS ORQALI TO'LOV (Mini App ichidan) ----------

STARS_COIN_PACKAGES = {
    "100_5000": (100, 5000),
    "300_16000": (300, 16000),
    "500_28000": (500, 28000),
    "1000_60000": (1000, 60000),
}
...
async def api_buy_stars_coins(request: web.Request):
    ...
    if package not in STARS_COIN_PACKAGES:
        return web.json_response({"error": "invalid_package"}, status=400)

    stars, coins = STARS_COIN_PACKAGES[package]
    ...
    try:
        ...
        logger.exception("create_invoice_link (coins) xatosi")
...
async def api_buy_stars_premium(request: web.Request):
    ...
    try:
        ...
        logger.exception("create_invoice_link (premium) xatosi")
        return web.json_response({"error": "invoice_failed"}, status=500)
