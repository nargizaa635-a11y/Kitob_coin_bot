...
async def _download_audio_chapter(message: Message, tmp_dir: str, chapter_index: int):
    """Audio yoki hujjat (mp3) xabaridan bob faylini yuklab oladi. (path, title, duration_seconds) qaytaradi."""
    if message.audio:
        src = message.audio
...
@dp.message(AddAudioBookStates.chapters, F.audio | F.document)
async def process_audio_book_chapter_file(message: Message, state: FSMContext):
    ...
    if result == "too_big":
        return
    if result is None:
        await message.answer("❗ Iltimos, audio (🎵) yoki MP3 fayl (📎) yuboring, yoki /tayyor / /bekor.")
        return
