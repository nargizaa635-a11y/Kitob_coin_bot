...
def build_avatar_url(user_id: int, avatar_path) -> str:
    """Avatar URL'iga fayl vaqtiga asoslangan versiya belgisini qo'shadi —
    shunda rasm qayta yuklanganda ilova/Telegram uni keshdan emas, yangidan oladi."""
    if not avatar_path:
        return None
    full_path = os.path.join(BOOK_STORAGE_DIR, avatar_path)
    try:
        version = int(os.path.getmtime(full_path))
    except OSError:
        version = 0
    return f"/api/avatar/{user_id}?v={version}"
...
async def api_me(request: web.Request):
    ...
    return web.json_response({
        ...
        "avatar_url": build_avatar_url(user_id, user["avatar_path"]),
        ...
        "leaderboard": [
            {
                ...
                "avatar_url": build_avatar_url(r['user_id'], r.get("avatar_path")),
            }
...
async def api_user_profile(request: web.Request):
    ...
    profile["avatar_url"] = build_avatar_url(target_user_id, profile.get("avatar_path"))
    ...
    coins_list = [
        {
            ...
            "user_id": r["user_id"],
            "avatar_url": build_avatar_url(r['user_id'], r["avatar_path"]),
...
async def api_rank_gap(request: web.Request):
    """Foydalanuvchining tanlangan reyting turidagi o'rnini va undan bitta
    yuqoridagi odamgacha qancha qolganini qaytaradi (motivatsion karta uchun)."""
    init_data = request.query.get("initData", "")
    user_data = validate_init_data(init_data)
    if not user_data:
        return web.json_response({"error": "unauthorized"}, status=401)
    ...
    user_id = user_data["id"]
    rank_type = request.query.get("type", "coins")

    def display_name(username, uid):
        if username and not str(username).isdigit():
            return f"@{username}"
        return f"ID{str(uid)[-4:]}"

    if rank_type == "referrals":
        gap = await db.get_referral_rank_gap(user_id)
        unit = "referral"
    elif rank_type == "quiz":
        gap = await db.get_quiz_rank_gap(user_id)
        unit = "ball"
    else:
        gap = await db.get_coins_rank_gap(user_id)
        unit = "koin"

    if not gap:
        return web.json_response({"has_gap": False})

    return web.json_response({
        "has_gap": True,
        "my_rank": gap["my_rank"],
        "my_value": gap["my_value"],
        "next_value": gap["next_value"],
        "next_name": display_name(gap["next_username"], 0),
        "gap": gap["gap"],
        "unit": unit,
    })
...
def create_app() -> web.Application:
    ...
    app.router.add_get("/api/rank_gap", api_rank_gap)
