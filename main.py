...
# Dostavka narxi hududga emas, balki buyurtma summasiga bog'liq (pochta punktiga yetkazish tarifi)
DELIVERY_TIERS = [
    (20000, 15000),
    (70000, 5000),
    (90000, 3000),
]
DELIVERY_FREE_THRESHOLD = 90000


def calculate_delivery_cost(product_price: int) -> int:
    for max_price, cost in DELIVERY_TIERS:
...
UZBEKISTAN_REGIONS = [
    ...
    "Buxoro viloyati", "Navoiy viloyati", "Qashqadaryo viloyati", "Surxondaryo viloyati",
    "Xorazm viloyati", "Qoraqalpog'iston Respublikasi",
]

# Faqat shu kategoriyadagi mahsulotlar jismoniy yetkazib berishni talab qiladi
PHYSICAL_DELIVERY_CATEGORIES = {"book", "stationery", "other"}
        ...
        if notify:
            try:
                ...
                await bot.send_message(
                    user_id,
                    f"🎉 Tabriklaymiz! Siz <b>{new_level}-darajaga</b> ko'tarildingiz!\n"
...
async def cmd_admin(message: Message):
    ...
    await message.answer(
        ...
        f"<code>/kitob_ochir ID</code> — kitobni o'chirish\n\n"
        f"✅ Vazifalar:\n"
...
async def _send_orders_list(message: Message, status: str):
    orders = await db.get_orders_by_status(status)
    label = ORDER_STATUS_LABELS.get(status, status)
    if not orders:
        await message.answer(f"{label} holatidagi buyurtmalar yo'q.")
        return
    lines = [f"{label} buyurtmalar ({len(orders)} ta):\n"]
    for o in orders:
        lines.append(
            f"№{o['id']} — {o['product_name']} — {o['total_amount']:,} so'm\n"
            f"   👤 {o['buyer_name'] or '—'} | 📞 {o['contact_phone'] or '—'}\n"
            f"   🏙 {o['region'] or '—'}, {o['district'] or '—'}"
        )
    text = "\n\n".join(lines)
...
@dp.message(Command("buyurtmalar_yangi"))
async def cmd_buyurtmalar_yangi(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _send_orders_list(message, "yangi")


@dp.message(Command("buyurtmalar_tolangan"))
async def cmd_buyurtmalar_tolangan(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _send_orders_list(message, "tolandi")


@dp.message(Command("buyurtmalar_tayyor"))
async def cmd_buyurtmalar_tayyor(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _send_orders_list(message, "tayyorlanmoqda")


@dp.message(Command("buyurtmalar_yetkazildi"))
async def cmd_buyurtmalar_yetkazildi(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _send_orders_list(message, "yetkazildi")
...
@dp.message(F.text.regexp(r"^/buyurtma_holat_(\d+)\s+(\S+)$"))
async def cmd_buyurtma_holat(message: Message):
    if not is_admin(message.from_user.id):
        return
    match = re.match(r"^/buyurtma_holat_(\d+)\s+(\S+)$", message.text)
    order_id = int(match.group(1))
    raw_status = match.group(2).lower()
    status = _ORDER_STATUS_ALIASES.get(raw_status)
    if not status:
        await message.answer(
            "❗ Noma'lum holat. Quyidagilardan birini yozing:\n"
            "<code>yangi</code>, <code>tolandi</code>, <code>tayyorlanmoqda</code>, "
            "<code>yetkazildi</code>, <code>bekor</code>"
        )
        return
    ...
    if not ok:
        await message.answer("❌ Holatni o'zgartirib bo'lmadi.")
        return

    await message.answer(f"✅ Buyurtma №{order_id} holati: {ORDER_STATUS_LABELS[status]}")

    try:
        status_text_for_buyer = {
            "tolandi": "💳 To'lovingiz qabul qilindi!",
            "tayyorlanmoqda": "📦 Buyurtmangiz tayyorlanmoqda.",
            "yetkazildi": "✅ Buyurtmangiz yetkazib berildi. Xaridingiz uchun rahmat!",
            "bekor_qilingan": "❌ Buyurtmangiz bekor qilindi. Savol bo'lsa, admin bilan bog'laning.",
        }.get(status)
        if status_text_for_buyer:
            await bot.send_message(
                order["user_id"],
                f"{status_text_for_buyer}\n\n🛍 {order['product_name']} — №{order_id}"
            )
    except Exception:
        pass
...
async def cmd_challenge_yarat(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📚 Yangi Challenge yaratamiz.\n\nAvval kitob muqovasi rasmini yuboring:"
    )
...
async def process_challenge_file(message: Message, state: FSMContext):
    ...
    if parsed["top_bonuses"]:
        await db.set_challenge_top_bonuses(challenge_id, parsed["top_bonuses"])
    else:
        await db.set_challenge_top_bonuses(challenge_id, [
            {"rank": 1, "coin": 300, "xp": 150},
            {"rank": 2, "coin": 200, "xp": 100},
            {"rank": 3, "coin": 150, "xp": 75},
            {"rank": 4, "coin": 100, "xp": 50},
            {"rank": 5, "coin": 100, "xp": 50},
        ])
...
async def cmd_challenge_yakunlash(message: Message):
    ...
    leaderboard = await db.get_challenge_leaderboard(challenge_id, limit=5)
    bonuses = await db.get_challenge_top_bonuses(challenge_id)
    bonus_map = {b["rank"]: b for b in bonuses}

    for idx, participant in enumerate(leaderboard, start=1):
        bonus = bonus_map.get(idx)
        if bonus:
            await db.add_coins(participant["user_id"], bonus["coin_reward"])
            lvl_info = await award_xp_and_level_rewards(participant["user_id"], bonus["xp_reward"], notify=False)
            level_note = ""
            if lvl_info.get("level_up_coins"):
                badges_note = ("\n🏅 Yangi nishon: " + ", ".join(lvl_info["new_badges"])) if lvl_info["new_badges"] else ""
                level_note = (
                    f"\n\n🎉 Siz <b>{lvl_info['level']}-darajaga</b> ko'tarildingiz! "
                    f"🪙 +{lvl_info['level_up_coins']} qo'shimcha koin!{badges_note}"
                )
            try:
                await bot.send_message(
                    participant["user_id"],
                    f"🏆 Tabriklaymiz! Siz \"{challenge['book_title']}\" challenge'ida "
                    f"{idx}-o'rinni egalladingiz!\n"
                    f"🪙 +{bonus['coin_reward']} koin, ⚡ +{bonus['xp_reward']} XP mukofot sifatida berildi!"
                    f"{level_note}"
                )
            except Exception:
                pass
    ...
    await message.answer(
        ...
        f"🏆 TOP {len(leaderboard)} kishiga mukofot tarqatildi."
    )
...
async def api_buy_shop_product(request: web.Request):
    ...
    if payment_type == "uzs" and product["category"] in PHYSICAL_DELIVERY_CATEGORIES:
        return web.json_response({"error": "needs_delivery_form"}, status=400)
...
async def api_checkout_info(request: web.Request):
    """Buyurtma formasi ochilishidan oldin kerakli ma'lumot: mahsulot narxi,
    hisoblangan dostavka narxi va viloyatlar ro'yxati."""
    init_data = request.query.get("initData", "")
    user_data = validate_init_data(init_data)
    if not user_data:
        return web.json_response({"error": "unauthorized"}, status=401)
    ...
    price = product["price_uzs"] or 0
    delivery_cost = calculate_delivery_cost(price)
...
async def api_create_order(request: web.Request):
    ...
    referral_count = await db.get_referral_count(user_id)
    if referral_count < product["required_referrals"]:
        return web.json_response({"error": "not_enough_referrals"}, status=400)

    buyer_name = (body.get("buyer_name") or "").strip()
    phone = (body.get("phone") or "").strip()
    region = (body.get("region") or "").strip()
    district = (body.get("district") or "").strip()
    address_note = (body.get("address_note") or "").strip() or None
    lat = body.get("lat")
    lng = body.get("lng")

    if not buyer_name or not phone or not region or not district:
        return web.json_response({"error": "missing_fields"}, status=400)
    if not address_note and (lat is None or lng is None):
        return web.json_response({"error": "missing_location"}, status=400)

    # Narx va dostavka HAR DOIM serverda qayta hisoblanadi — mijozdan kelgan
    # qiymatga ishonilmaydi (aks holda odam narxni o'zgartirib yuborishi mumkin edi)
    price = product["price_uzs"] or 0
    delivery_cost = calculate_delivery_cost(price)

    user = await db.get_user(user_id)
    order_id = await db.create_delivery_order(
        user_id, product_id, price, delivery_cost,
        buyer_name=buyer_name, contact_phone=phone, region=region, district=district,
        location_lat=lat, location_lng=lng, address_note=address_note,
        contact_username=user["username"] if user else None,
    )
    loyalty_bonus = await apply_shop_loyalty_bonus(user_id)
    ...
    total = price + delivery_cost
    location_line = (
        f"📍 <a href='https://maps.google.com/?q={lat},{lng}'>Joylashuv (xaritada ko'rish)</a>"
        if lat is not None and lng is not None
        else f"📍 Mo'ljal: {address_note}"
    )
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"🚚 Dostavka: {delivery_cost:,} so'm\n"
                f"💵 Jami: {total:,} so'm\n\n"
                f"👤 Xaridor: {buyer_name} (@{user['username'] if user and user['username'] else '—'}, "
                f"ID: <code>{user_id}</code>)\n"
                f"📞 Telefon: {phone}\n"
                f"🏙 {region}, {district}\n"
                f"{location_line}\n\n"
                f"Holatni o'zgartirish: <code>/buyurtma_holat_{order_id} tolandi</code>",
    ...
    return web.json_response({
        ...
        "price_uzs": price, "delivery_cost": delivery_cost, "total": total,
...
async def api_challenge(request: web.Request):
    ...
    day_count = await db.get_active_challenge()
    ...
    if participant:
        ...
        if participant["current_day"] <= challenge["days_count"]:
            progress = await db.get_daily_progress(participant["id"], participant["current_day"])
            if progress:
                # Bugungi kun allaqachon bajarilgan, 24 soat qulf hisoblanadi
...
async def api_challenge_join(request: web.Request):
    ...
    return web.json_response({"success": True, "participant_id": participant["id"]})
...
async def api_challenge_day(request: web.Request):
    ...
    progress = await db.get_daily_progress(participant["id"], day_number)
    if progress:
        completed_dt = datetime.strptime(progress["completed_at"], "%Y-%m-%d %H:%M")
        unlock_at = completed_dt + timedelta(hours=CHALLENGE_DAY_UNLOCK_HOURS)
...
def create_app() -> web.Application:
    ...
    app.router.add_get("/api/checkout_info", api_checkout_info)
