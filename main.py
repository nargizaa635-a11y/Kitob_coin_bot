...
ADMIN_IDS = [8241010228, 1920583847]
...
async def cmd_admin(message: Message):
    ...
    await message.answer(
        ...
        f"<code>/kitoblar</code> — barcha kitoblar va ID'lari ro'yxati\n"
        ...
        f"💰 Moliyaviy va statistik hisobotlar:\n"
...
@dp.message(Command("kitoblar"))
async def cmd_kitoblar(message: Message):
    if not is_admin(message.from_user.id):
        return
    books = await db.get_all_books()
    if not books:
        await message.answer("Hozircha kitob yo'q.")
        return
    lines = ["📚 <b>Kitoblar ro'yxati:</b>\n"]
    for b in books:
        lines.append(
            f"#{b['id']} — <b>{b['title']}</b> ({b['author']})\n"
            f"   🏷 {b['genre']} | 📄 {b['page_count']} sahifa | "
            f"👥 {b['required_referrals']} referral | 🪙 {b['required_coins']:,} koin"
        )
    lines.append(
        "\nO'chirish uchun:\n<code>/kitob_ochir ID</code>\n"
        "Masalan: <code>/kitob_ochir 3</code>"
    )
    await message.answer("\n".join(lines))
...
async def cmd_sandiq_yarat(message: Message):
    ...
    try:
        ...
        if skipped:
            await message.answer(
                "❗ Quyidagi mukofot(lar) tanilmadi (\"coins\" yoki \"premium\" bilan boshlanishi kerak): "
                + "; ".join(skipped)
            )
            return
...
async def cmd_challenge_yakunlash(message: Message):
    ...
    for idx, participant in enumerate(leaderboard, start=1):
        ...
        if bonus:
            ...
            await db.add_xp(participant["user_id"], bonus["xp_reward"])
            try:
                await bot.send_message(
                    participant["user_id"],
...
async def api_challenge_day_submit(request: web.Request):
    ...
    xp_info = await db.add_xp(user_id, xp_earned)
...
async def api_book_page_data(request: web.Request):
    ...
    if page_type == "text":
        return web.json_response({"type": "text", "content": page["text_content"] or ""})
    return web.json_response({
        "type": "image",
        "image_url": f"/api/page_image/{book_id}/{page_number}?initData={quote(init_data, safe='')}",
    })
