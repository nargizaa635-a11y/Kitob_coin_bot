...
def calculate_coin_price(cost_usd: float, margin: float = DEFAULT_MARGIN) -> int:
    """Tannarx ($) + marja → Koin narxi"""
    if cost_usd <= 0:
        return 0
    target_net = float(cost_usd) * (1 + margin)
    stars_needed = target_net / NET_USD_PER_STAR
    coins = int(round(stars_needed * STAR_TO_COIN))
    return max(coins, 100)
...
async def add_shop_product(name: str, description: str, category: str, cost: int,
                            required_referrals: int, stock, cost_usd: float = 0) -> int:
    async with _pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO shop_products
                (name, description, category, image_path, cost, required_referrals, stock, cost_usd, active, created_at)
