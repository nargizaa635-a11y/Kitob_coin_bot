...
async def init_db():
    ...
    async with _pool.acquire() as conn:
        ...
        # ---------- HAQIQIY DO'KON (jismoniy mahsulotlar: Premium, Gift, kitob, kantselyariya) ----------
        await conn.execute("""
        ...
        # Admin test/demo maqsadida o'ziga (yoki boshqa foydalanuvchiga) sun'iy
        # referral qo'sha olishi uchun — haqiqiy referal yozuvlarini buzmasdan
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS bonus_referrals INTEGER DEFAULT 0")

        # ---------- MAJBURIY OBUNA KANALLARI (bot buyruqlari orqali boshqariladi) ----------
        await conn.execute("""
...
async def add_bonus_referrals(user_id: int, amount: int) -> int:
    """Admin test/demo maqsadida sun'iy referral qo'shadi. Yangi umumiy
    (haqiqiy + bonus) referral sonini qaytaradi."""
    async with _pool.acquire() as conn:
        await conn.execute(
...
async def get_quiz_leaderboard(limit: int = 10):
    async with _pool.acquire() as conn:
        return await conn.fetch(
            "SELECT user_id, username, avatar_path, quiz_score_total AS cnt FROM users "
...
async def get_coins_leaderboard(limit: int = 10):
    async with _pool.acquire() as conn:
        return await conn.fetch(
            "SELECT user_id, username, avatar_path, coins FROM users ORDER BY coins DESC LIMIT $1", limit
        )
...
async def get_referral_leaderboard(limit: int = 10):
    async with _pool.acquire() as conn:
        return await conn.fetch(
            ...
            FROM users u
            LEFT JOIN users r ON r.referred_by = u.user_id
            GROUP BY u.user_id, u.username, u.avatar_path, u.bonus_referrals
            HAVING COUNT(r.user_id) + COALESCE(u.bonus_referrals, 0) > 0
            ORDER BY cnt DESC
            LIMIT $1
            """,
            limit,
...
# ---------- HAQIQIY DO'KON (jismoniy mahsulotlar) ----------

async def add_shop_product(name: str, description: str, category: str, cost: int,
                            required_referrals: int, stock) -> int:
    """`stock` — None bo'lsa cheksiz, aks holda dona soni."""
    async with _pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO shop_products
                (name, description, category, image_path, cost, required_referrals, stock, active, created_at)
            VALUES ($1, $2, $3, '', $4, $5, $6, TRUE, $7)
            RETURNING id
            """,
            name, description, category, cost, required_referrals, stock,
            datetime.now().strftime("%Y-%m-%d %H:%M"),
        )
        return row["id"]
...
async def restock_shop_product(product_id: int, new_stock):
    """`new_stock` — None bo'lsa cheksiz qilib belgilaydi."""
    async with _pool.acquire() as conn:
        await conn.execute(
...
async def record_shop_purchase(user_id: int, product_id: int, cost_paid: int):
    async with _pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO shop_purchases (user_id, product_id, cost_paid, status, purchased_at)
...
async def remove_force_channel(channel_username: str) -> bool:
    async with _pool.acquire() as conn:
        result = await conn.execute(
            "UPDATE force_channels SET active = FALSE WHERE channel_username = $1 AND active = TRUE",
            channel_username,
        )
...
async def add_quiz_questions(quiz_set_id: int, questions: list):
    """`questions` — har biri dict: {question, a, b, c, d, correct} ('A'/'B'/'C'/'D')."""
    async with _pool.acquire() as conn:
...
# ---------- OCHIQ (BOSHQA FOYDALANUVCHI) PROFILI ----------
